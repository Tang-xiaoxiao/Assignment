# ============================================================
# 功能说明：
# 本脚本用于训练一个基于 T5 的多模态生成模型（T5ForMultimodalGeneration），
# 主要应用于医学视觉问答（Medical VQA）任务。模型能够同时接收文本信息和图像特征，
# 根据不同训练方法生成答案、解释或推理结果。
#
# 主要流程：
# 1. 设置随机种子，保证训练结果尽可能可复现；
# 2. 从预训练模型路径加载多模态 T5 模型；
# 3. 对新增的多模态模块参数进行稳定初始化，避免训练初期数值不稳定；
# 4. 加载 T5 tokenizer 和数据整理器 DataCollatorForSeq2Seq；
# 5. 配置 HuggingFace 的 Seq2SeqTrainingArguments 训练参数；
# 6. 构建训练数据集 ClosedMedVQADataset；
# 7. 使用 Seq2SeqTrainer 执行训练；
# 8. 将训练完成后的模型保存到指定目录。
#
# 适用任务：
# - Explanation              : 生成解释
...
# - without_R                : 不使用推理模块的训练方式
#
# 模型特点：
# - 基于预训练 T5 扩展得到；
# - 支持融合图像特征与文本输入；
# - 对自定义多模态层（如 gate_dense、image_dense、mha_layer）进行重新初始化；
# - 初始化时使用均值 0、标准差 0.02 的正态分布，以提高训练稳定性。
#
#
# 输入参数说明：
# --train_text_file_path   : 训练文本数据路径
# --img_file_path          : 图像特征文件路径（通常为 .pth）
# --img_name_map           : 图像名到特征索引的映射文件路径（通常为 .json）
# --pretrained_model_path  : 预训练 T5 或多模态模型路径
# --output_dir             : 训练输出目录
# --method                 : 训练方法类型，决定任务模式
# --source_len             : 输入文本最大长度
# --target_len             : 输出文本最大长度
# --lr                     : 学习率
# --epoch                  : 训练轮数
...
# --rational               : 是否使用 ROUGE-L 作为生成任务评价指标
#
# 输出结果：
# - 在 output_dir/method/ 目录下保存训练后的模型参数；
#
# ============================================================
import nltk
import evaluate
...
import numpy as np
import torch
from transformers import T5Tokenizer, Seq2SeqTrainingArguments, Seq2SeqTrainer, DataCollatorForSeq2Seq
...
def train_loop(_args):
    ...
    # ========== 最小化修改：全新自定义层数值稳定初始化 ==========
    print("Stabilizing newly initialized custom multimodal layers...")
    for name, param in model.named_parameters():
        if any(custom_key in name for custom_key in ["gate_dense", "image_dense", "mha_layer"]):
            if param.dim() >= 2:
                # 使用 Transformer 标准的 0.02 小标准差重新初始化权重，彻底杜绝 Softmax 溢出
                torch.nn.init.normal_(param.data, mean=0.0, std=0.02)
                print(f"  - Re-initialized weight matrix: {name} to std=0.02")
            elif "bias" in name:
                torch.nn.init.zeros_(param.data)
                print(f"  - Re-initialized bias: {name} to zeros")
    print("Custom layers stabilized successfully!\n")
    # ==========================================================

    tokenizer = T5Tokenizer.from_pretrained(_args.pretrained_model_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rational', action='store_true', default=False, help='Whether to use ROUGE-L as metric for rationale generation')
